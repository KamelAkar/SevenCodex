...
function renderAutoLoadFooter(nextHref, nextLimit, shown, total) {
  if (!nextHref || !nextLimit) return "";
  return `<div id="autoLoadSentinel" class="auto-load-sentinel" data-next-limit="${escapeHtml(String(nextLimit))}"><span class="tag auto-load-count">${entriesShownLabel(shown, total)}</span>${button(nextHref, t(state.language, "actions.showMore"))}</div>`;
}
...
export function renderList(store, route) {
  ...
  const nextLimit = visibleEntries.length < entries.length ? Math.min(entries.length, limit + LIST_PAGE_SIZE) : 0;
  const opts = optionsForEntries(base);
  ...
  const activeFilters = [
    ...
    .filter(Boolean)
    .join("");
  const showMoreHref = nextLimit ? buildRouteUrl({ ...route, limit: String(nextLimit) }) : "";
